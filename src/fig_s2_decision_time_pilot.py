...
def _plot_observed(data, filename, ylim=15, inset_pos=None):
    if inset_pos is None:
        inset_pos = [0.55, 0.6, 0.38, 0.38]

    fig, ax = plt.subplots(
        figsize=(FIG_WIDTH_IN * 0.3, FIG_WIDTH_IN * 0.25),
...
def _plot_recovered(data, filename):
    ...
    inset = _setup_inset(ax, [0.55, 0.6, 0.38, 0.38])
...
def main():
    ...
    out = OUTPUT_FIGURES / "fig_s2_decision_time_pilot.pdf"
    fig.savefig(out)
    print(f"Saved to {out}")
